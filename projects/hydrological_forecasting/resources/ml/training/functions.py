...
class post_processing():
    ...
    storica in ballo"""  
    def plot_complete_prediction(self, interactive=True, save=True):     
        ...
        if interactive == False:   
            ...
            ax.plot(self.df_real, label = 'True')
            ax.plot(self.df_train, alpha=0.4, label = 'train')
            ax.plot(self.df_test, color='green', alpha=0.4, label = 'test')
            ...
            fig.autofmt_xdate()
            if save==True:
                plt.savefig(self.model.save_path+'/'+'complete.png', dpi=300, bbox_inches='tight')
            # plt.show()
        else:
            ...
            trace0 = go.Scatter(x=self.df_real.index, y=self.df_real["h average"], name="metered")
            trace1 = go.Scatter(x=self.df_real.index, y=self.df_train["h average"], name="training")
            trace2 = go.Scatter(x=self.df_real.index, y=self.df_test["h average"], name="testing")
            trace3 = go.Scatter(x=self.model.df_multi_rain.index, y=self.model.df_multi_rain['h average rain'], name="precipitation")
            ...
            fig.write_html(self.model.save_path+'/'+'complete.html')
        
    """Fuzione per fare i plot di solo una parte della time series"""    
    def plot_partial_prediction(self, start_date, n_days, y0=None,
                                y1 =None, save=False):
        start_date = datetime.datetime.strptime(start_date, '%Y-%m-%d')
        end_date = start_date + datetime.timedelta(days = n_days)
        fig, (ax, ax1) = plt.subplots(2, figsize=(15, 7),
        ...
        ax.plot(self.df_real[start_date:end_date], label = 'True')
        ax.plot(self.df_train[start_date:end_date])
        ax.plot(self.df_test[start_date:end_date], label = 'prediction')
        if y0!=None:
            ax.set_ylim([y0,y1])
        # ax.set_xlabel('hour')
        ...
        ax1.plot(self.model.df_multi_rain[start_date:end_date]['h average rain'],
                 label = 'rain', color ='k')
        ax1.legend()
        ax1.grid(True) 
        ax1.set_ylabel('(mm)')
        ...
        if save==True:
            plt.savefig(self.model.save_path+'/'+'partial.png', dpi=300, bbox_inches='tight')
        # plt.show()
    ...
    def plot_multiple_rolled_prediction(self, df_roll, figsize, save=None):
        ...
        fig.autofmt_xdate()
        if save==True:
            plt.savefig(self.model.save_path+'/'+'plot_multiple_rolled_prediction.png', dpi=300,
                        bbox_inches='tight')
        # plt.show() 
        return df
    ...
    def plot_boxplot(self, time, error, dataset=None):
        ...
        if time=='hourly':
            seaborn.boxplot(x = df.index.hour, y = df['h average'],
                            color="lightblue", ax=ax)
        elif time == 'daily':
            seaborn.boxplot(x = df.index.dayofweek, y = df['h average'],
                            color="lightblue", ax=ax)
        elif time == 'monthly':
            seaborn.boxplot(x = df.index.month, y = df['h average'],
                            color="lightblue", ax=ax)
        ax.set(xlabel=time, ylabel=error)
        # ax.set_ylim([-0.5,0.5])
        ax.axhline(y=0, xmin=-1, xmax=32)
        plt.savefig(self.model.save_path+'/'+'box.png', dpi=300)
        # plt.show() 
    ...
    def plot_differences(self, time, year=None):
        ...
        ax.grid(True)       
        fig.autofmt_xdate()
        plt.savefig(self.model.save_path+'/'+'differ.png', dpi=300)
        # plt.show()
    ...
    def mean_absolute_percentage_error(self, y_true, y_pred): 
        ...
        y_true = np.array(list(map(lambda x: np.NaN if x == 0 else x, y_true)))
        y_pred = np.array(list(map(lambda x: np.NaN if x == 0 else x, y_pred)))
    ...
    def saver_result(self):
        if self.model.do_you_want_to_save == True:
            ...
            newdf.to_csv(self.model.save_path+'/'+'/results.csv', sep=';', encoding='utf-8', index=True)
        else:
            pass