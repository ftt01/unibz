...
class Dataset_management():
    ...
    def parsing_idro(self, start=None, end=None, ens_id=None):
        if ens_id != None:
            ...
            if ens_id<10:
                ...
                name = '0' + str(ens_id)
            self.df = pd.DataFrame(self.df[name])
            self.df = self.df.rename(columns = {name: 'ts'}) 
        else:
            try:
                self.df = pd.read_csv(self.string, sep=';')
                self.df.index = pd.to_datetime(self.df['datetime'],
                ...
                self.df = self.df.rename(columns = {'001': 'ts'}) 
            except KeyError:
                try:
                    self.df = pd.read_csv(self.string, sep=',')
                    self.df.index = pd.to_datetime(self.df['datetime'],
        ...
        if start!=None and end!= None:
            ...
            self.df.loc[end] = 0
            self.df = self.df.resample('H').bfill()
        return self.df
