...
class Model_generator():
    ...
    def __init__(self, dataset, model_type, data_manager, univariate_scaler,
                       multivariate_scaler, n_output, manager):
    ...
    def lag_selector(self, other_var=None, lista_lag=None, n_output=None, 
                     ...
                     day_cumsum_horiz_rain=None, ens_id = None):
    ...
    def add_rainfall(self, rainfall, lags, day_cumsum, day_cumsum_horiz, ens_id):
        if rainfall==True:
            Manager_rain = Instances.management_instance(string = self.manager.string_rain, 
                                  string_rain = None,
                                  string_temp=None,
            ...
            self.model_rain = Model_generator(Manager_rain.dataset_univariate,
                                     model_type=None,
    ...
    def add_temperature(self, temperature, lags, ens_id):
        if temperature==True:
            Manager_temp = Instances.management_instance(string = self.manager.string_temp, 
                                  string_rain = None,
                                  string_temp=None,
                                  weekend_exclusion=self.manager.w_ex, 
                                  year_exclusion = self.manager.year_exclusion,
                                  ...
                                  ens_id = ens_id)
            # creo un odello fittizzio solo per usare i metodi di questa classe
            self.model_temp = Model_generator(Manager_temp.dataset_univariate,
                                     model_type=None,
    ...
    def add_rain_forecast(self, rainfall, rain_for_horizon, ens_id):
        if rainfall==True:
            Manager_rain = Instances.management_instance(string = self.manager.string_rain, 
                                  string_rain = None,
                                  string_temp=None,
            ...
            self.model_rain_forec = Model_generator(Manager_rain.dataset_univariate,
                                     model_type=None,
                                     data_manager = Manager_rain.d_m,
                                     univariate_scaler = Manager_rain.univariate_scaler,
                                     multivariate_scaler = Manager_rain.multivariate_scaler,
    ...
    """Funzione per dataset multivariato. variabili pioggia previsionali"""    
    def add_temp_forecast(self, temperature, temp_for_horizon, ens_id):
        if temperature==True:
            Manager_temp = Instances.management_instance(string = self.manager.string_rain, 
                                  string_rain = None,
                                  string_temp=None,
                                  weekend_exclusion=self.manager.w_ex, 
                                  year_exclusion = self.manager.year_exclusion,
                                  ...
                                  ens_id = ens_id)
            # creo un modello fittizzio solo per usare i metodi di questa classe
            self.model_temp_forec = Model_generator(Manager_temp.dataset_univariate,
                                     model_type=None,
                                     data_manager = Manager_temp.d_m,
                                     univariate_scaler = Manager_temp.univariate_scaler,
                                     multivariate_scaler = Manager_temp.multivariate_scaler,
    ...
    def model_creation(self, train_X,  n_node, n_layer, act_function=None):
        self.n_node = n_node
        self.n_layer = n_layer
        self.act_function = act_function
        if self.model_type == 'SVR':
            self.model = SVR_generator().SVR_model(self.model_type, self.n_output)
        elif self.model_type == 'MultiOutputSVR':
            self.model = SVR_generator().MultiOutputSVR_model(self.model_type)
        elif self.model_type == 'LinearRegression':
            self.model = SVR_generator().LinearRegression_model(self.model_type)
        elif self.model_type == 'KNeighborsRegressor':
            self.model = SVR_generator().KNeighborsRegressor_model(self.model_type)
        elif self.model_type == 'DecisionTreeRegressor':
            self.model = SVR_generator().DecisionTreeRegressor_model(self.model_type)
        elif self.model_type == 'SARIMAX':
            pass
        else:
            self.model = Deep_learning().Deep_learning_model(model_type=self.model_type, 
                                                             train_X=train_X,  
                                                             n_node=self.n_node, 
    ...
    def model_saver(self, do_you_want_to_save, error_string = None):
        ...
        if do_you_want_to_save == True:
            # nome del file
            basename = "logfile"
            self.suffix = datetime.datetime.now().strftime("%d_%m_%Y at %Hh %Mm %Ss")
            logfile = " ".join([basename, self.suffix])
            # creo i contenuti del logfile
            log_string = \
            'model generated in ' + self.suffix + '\n\n' + \
            ...
            # creo la prima cartella con la sola data
            self.first_path = datetime.datetime.now().strftime("%d_%m_%Y")
            if not os.path.exists('Model_repository/' + self.first_path):
                os.makedirs('Model_repository/' + self.first_path)
            # path completo con la seconda nuova folder
            self.newpath = 'Model_repository/' + self.first_path + '/'  + self.suffix
            os.makedirs(self.newpath)
            # salvo il logfile che ho generato come stringa
            with open(self.newpath + '/' + logfile + '.txt', 'w') as tfile:
                tfile.write(log_string)
            #salvo il modello
            if self.bagging == True:
                i=1
                for model in self.bootsrap_ensamble:
                    os.makedirs(self.newpath + '/' + str(i))
                    if self.model_type == 'SVR' or self.model_type == 'LinearRegression' \
                        or self.model_type == 'KNeighborsRegressor' \
                            or self.model_type == 'DecisionTreeRegressor' \
                                or self.model_type == 'MultiOutputSVR':
                        joblib.dump(model, self.newpath + '/' + self.model_type + '_model.joblib')
                    else:
                        model.save(self.newpath + '/' + str(i) + '/' + self.model_type + '_model.h5')
                    i = i+1
            else:
                if self.model_type == 'SVR' or self.model_type == 'LinearRegression' \
                    or self.model_type == 'KNeighborsRegressor' \
                        or self.model_type == 'DecisionTreeRegressor' \
                            or self.model_type == 'MultiOutputSVR':
                    joblib.dump(self.model, self.newpath + '/' + self.model_type + '_model.joblib')
                else:
                    self.model.save(self.newpath + '/' + self.model_type + '_model.h5')
        else:
            pass
