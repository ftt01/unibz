...
from lib import dt, pd, np, plt, time, logging

from functions import Optimisation_functions
from functions import Output_collect
from functions import create_directory_tree
...
def regularizer(Q_constr_turb,
    ...
    logging.debug(f"optimized flow:\t {daily_turb}\n \
...
def regularizer2(Q_constr_turb, v_0, Q_in):
    ...
    if balance < 0:
        logging.warning("The basin is empty. Adjustment of the turbined volume")
...
def regularizer3(Q_constr_turb, v_0, Q_in, v_max, Q_max):
    ...
    balance = v_0 + daily_volume_inflow - daily_volume_constr_turb 
    if balance > v_max:
        logging.warning("The basin is full. Adjustment of the turbined volume")
...
def price_constr(
        Q_constr_turb, the_date, vol_resid, vol_max, vol_min, price, q_max_turb,
        prices_data, coeff_vol_intercept=0, coeff_vol_slope=2):
    
    econ_real = pd.read_csv(prices_data, sep=',', engine='python')
    ...
    try:
        p_ref = np.quantile(
            econ_real[the_date-dt.timedelta(days=30) :\
            the_date-dt.timedelta(days=1)].values, 0.5)
    except:
        logging.error("Not enough economic data to evaluate the constraint over last 30 days")
    ...
    coeff_volume = coeff_vol_intercept + coeff_vol_slope * (vol_resid-vol_max)/(vol_min - vol_max)
    ...
    logging.info(f"The adjusted constraint is: {Q_constr_turb}")
...
def var_saver(algorithm, output_path=None):
    Variabili = list()
    Obiettivi = list()
    ...
    if output_path is not None:
        output_filepath = output_path + 'Solution.csv'
...
def algorithm_creation(algo, pop, p_c, p_m, problem):
...
def main(
    ...
    evaluated_days = 0
    fail_count = 0
    collector = Output_collect(simulation_name)    # inizializzo l'oggetto collect
    
    while curr_date <= end_datetime:   
        logging.info(f"Optimisation launched with {meta_algorithm['name']}")
        logging.info(f"Current date: {curr_date}")
        # genero la classe funzioni
        functions = Optimisation_functions(
            curr_date, meta_basin['name'],
            paths['streamflow']['model'], paths['streamflow']['reference'], paths['price'] )
        # parsing dei dati
        idro, Q_out_constr, idro_real = functions.parsing_idro()
        if (isinstance(idro, type(None)) == True) or (np.isnan(Q_out_constr) == True):
            curr_date = curr_date + dt.timedelta(days=1)
            continue    

        logging.info(f"The original constraint is: {Q_out_constr}")
        econ = functions.parsing_economics()

        if (isinstance(econ, type(None)) == True):
            curr_date = curr_date + dt.timedelta(days=1)
            continue    
        ...
        Q_out_constr = price_constr(
            Q_constr_turb   = Q_out_constr,
            the_date        = curr_date,
            vol_resid       = meta_basin['v_0'],
            vol_max         = meta_basin['v_max'],
            vol_min         = meta_basin['v_min'], #v_max*0.8
            price           = econ,
            q_max_turb      = meta_basin['Q_max'],
            prices_data     = paths['price']
        )
      
        # verifico che il serbatoio non sia vuoto e in caso modifico il constr
        Q_out_constr = regularizer2(
            Q_constr_turb   = Q_out_constr,
            v_0             = meta_basin['v_0'], 
            Q_in            = idro['Q_in']
        )
        ...
        Q_out_constr = regularizer3(
            Q_constr_turb   = Q_out_constr,
            v_0             = meta_basin['v_0'], 
            Q_in            = idro['Q_in'],
            v_max           = meta_basin['v_max'],
            Q_max           = meta_basin['Q_max']
        )

        # inizializzo l'algoritmo
        # logging.debug(Q_out_constr)
        algorithm = algorithm_creation(
            meta_algorithm['name'], meta_algorithm['population'], 
            meta_algorithm['p_c'], meta_algorithm['p_m'],
            Hydro_optimization(
                meta_algorithm['n_var'], meta_algorithm['n_obj'], meta_algorithm['n_constr'], 
                meta_basin['Q_min'], meta_basin['Q_max'],
                meta_basin['h_min'], meta_basin['h_max'],
                idro['Q_in'], econ['values'].values,
                meta_basin['salto_mandata'], meta_basin['L_mandata'],
                meta_basin['ks_mandata'], meta_basin['D'],
                meta_basin['a'], meta_basin['b'],
                meta_basin['c'], meta_basin['d'],
                meta_basin['e'], meta_basin['f'],
                meta_basin['v_0'], meta_basin['v_max'], Q_out_constr))
        
        algorithm.run(meta_algorithm['n_gen'])

        logging.info(f"Elapsed time for simulation: {time.time() - t}")  

        # genero il file Solution.csv che contiene variabili e f_obj
        saved_solution = var_saver(algorithm, output_path=paths['output'])
        solutions = pd.read_csv(saved_solution, engine='python')
        # selezione delle solutions richiamando il metodo
        Q_out = solutions[solutions.columns[:23]]
        ...
        if cond == True:
            ...
            v_0 = functions.solution_selector(solutions, algorithm)
            output = functions.output_generator(algorithm, meta_algorithm['n_var'])
            collector.fill(output)
            curr_date = curr_date + dt.timedelta(days=1)
            evaluated_days = evaluated_days + 1
        # se la simulazione non ha rispettato il constraint ritenta
        elif cond == False:
            fail_count = fail_count+1
            logging.warning(f"Failed to converge. Fail number: {fail_count}")

    #salva tutto
    collector.saver(start_datetime, output_path=paths['output'])
    #genero gli output definitivi
    final_output = collector.list_return()
    logging.info(f"Optimisation ended with fails: {fail_count}")
    return algorithm, final_output, collector, evaluated_days
# ===================================================================
"""Main launcher"""
# ===================================================================
...
sim = {
    "start_datetime" : "20211001",
    "end_datetime" : "20211013",
}
...
paths = {
    "price" : '/home/daniele/documents/github/ftt01/phd/projects/optimization/data/input/prezzi_energia.csv',
    "streamflow" : {
        "reference" : '/home/daniele/documents/github/ftt01/phd/projects/optimization/data/input/dati_reali_scalati.csv',
        "model" : '/home/daniele/documents/github/ftt01/phd/projects/optimization/data/input/forecast_testing_scalati.csv'
    },
    "output" : '/home/daniele/documents/github/ftt01/phd/projects/optimization/data/output/'
}
...
create_directory_tree(paths['output'])

log_filename = f"{paths['output']}optimizer_{simulation_name}.log"
logging.basicConfig(
...
if __name__ == "__main__":
    ...
    # curr_date = pd.to_datetime(start_datetime, format="%Y%m%d")
    start_datetime = pd.to_datetime(sim['start_datetime'], format="%Y%m%d")
    end_datetime = pd.to_datetime(sim['end_datetime'], format="%Y%m%d")

    algorithm, final_output, collector, n_days = main(simulation_name, start_datetime, end_datetime,
                                              meta_basin, meta_algorithm, paths)

    output = pd.read_csv(collector.output_filepath, sep=';', engine='python')
    output.index = pd.to_datetime(output['Unnamed: 0'], format="%Y-%m-%d %H:%M:%S" )
    output.drop(columns=output.columns[[0]], inplace=True)
    
    Output_collect(simulation_name).complete_plotter(
        n_days=n_days, output=output, output_path=paths['output'])