...
output_log = output_path + 'AA_precipitation_lapse_rate_kriging11x8.log'
...
# path_in  = 'D:\\hydrology\\works\\calibration_validation\\hydro_modelling\\Passirio\\meteo\\kriging\\observations\\11x8\\precipitation\\'
path_in = 'D:\\hydrology\\results\\meteo\\kriging\\P\\'
...
precipitation_yearly_mean_mean = precipitation_yearly_mean
...
#         if elev[id] != -999:
#             # if prec > 400:
#             precipitation_df = precipitation_df.append({'id':str(id), 'elevation':elev[str(id)], 'precipitation': prec}, ignore_index=True)
# precipitation_df = precipitation_df.set_index('id')
...
logging.info( "Kriging 11x8 correlation: " + str(cc_p[0][1]) )
axs.plot( elevation, p1(elevation), "r--" )

cc_p  =  np.corrcoef(elevation, diff_p)

output_file_hd = output_path + 'meteo_' + basin + '_precipitation_kr11_over_elevation_hd.' + output_format
